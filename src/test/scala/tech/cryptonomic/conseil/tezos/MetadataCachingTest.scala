...
class MetadataCachingTest extends WordSpec with Matchers with OneInstancePerTest with OptionValues {
  ...
  "Metadata caching" should {
    ...
    "init attributes cache" in {
      val emptyAttributesCache: AttributesCache = Map("testNetwork" -> (0L, List()))
      sut.fillAttributesCache(emptyAttributesCache).unsafeRunSync()

      sut.getAttributes("not valid").unsafeRunSync() shouldBe None
      sut.getAttributes("testNetwork").unsafeRunSync() shouldBe Some(0L, List())
    }
    ...
    "init attribute values cache" in {
      ...
      val (_, result) = sut.getAttributeValues("table", "column").unsafeRunSync().value
      result.values.toList shouldBe List.empty
    }

    "insert/update values in entities cache" in {
      val emptyEntitiesCache: EntitiesCache = Map("testNetwork" -> (0L, List()))
      ...
      val (_, insertResult) = sut.getEntities("differentTestNetwork").unsafeRunSync().value
      insertResult shouldBe entitiesList

      // update
      sut.putEntities("differentTestNetwork", updatedEntityList).unsafeRunSync()
      val (_, updateResult) = sut.getEntities("differentTestNetwork").unsafeRunSync().value
      updateResult shouldBe updatedEntityList
      sut.getAllEntities.unsafeRunSync().mapValues(_._2) shouldBe Map("testNetwork" -> List(), "differentTestNetwork" -> List(Entity("x", "y", 0)))
    }

    "insert/update values in attributes cache" in {
      val emptyAttributesCache: AttributesCache = Map("testEntity" -> (0L, List()))
      ...
      val (_, insertResult) = sut.getAttributes("differentTestEntity").unsafeRunSync().value
      insertResult shouldBe attributesList

      // update
      sut.putAttributes("differentTestEntity", updatedAttributesList).unsafeRunSync()
      val (_, updateResult) = sut.getAttributes("differentTestEntity").unsafeRunSync().get
      updateResult shouldBe updatedAttributesList
      sut.getAllAttributes.unsafeRunSync().mapValues(_._2) shouldBe
        Map("testEntity" -> List(), "differentTestEntity" -> List(Attribute("x", "y", DataType.String, None, KeyType.NonKey, "z")))
    }
