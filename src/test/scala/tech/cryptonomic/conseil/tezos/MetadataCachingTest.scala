...
class MetadataCachingTest extends WordSpec with Matchers with OneInstancePerTest with OptionValues {
  ...
  "Metadata caching" should {
    ...
    "init attribute values cache" in {
      val attributeValuesCache: AttributeValuesCache = Map(CacheKey("table.column") -> CacheEntry(0L, RadixTree[String, String]()))
    ...
    "insert/update values in entities cache" in {
      ...
      sut.getAllEntities.unsafeRunSync().mapValues(_.value) shouldBe Map(CacheKey("testNetwork") -> List(), CacheKey("differentTestNetwork") -> List(Entity("x","y",0,None)))
    }
    ...
    "insert/update values in attributes cache" in {
      ...
      sut.getAllAttributes.unsafeRunSync().mapValues(_.value) shouldBe
        Map(CacheKey("testEntity") -> List(), CacheKey("differentTestEntity") -> List(Attribute("x","y",DataType.String,None,KeyType.NonKey,"z")))
    }

    "insert/update values in attribute values cache" in {
      val emptyAttributeValuesCache: AttributeValuesCache = Map(CacheKey("table.column") -> CacheEntry(0L, RadixTree[String, String]()))
