...
class NodeOperationsTest
    ...
    with Matchers
    with LazyLogging
    with NodeOperationsTestFixtures {

  // create a test instance
  val sut: NodeOperations = new NodeOperations(
    network = "network",
    batchConf = BatchFetchConfiguration(
      blockConcurrencyLevel = 100,
      accountConcurrencyLevel = 100,
      delegateConcurrencyLevel = 100,
      blockPageSize = 100,
      accountPageSize = 100
    )
  )
  ...
  "getBlock" should "correctly fetch the genesis block" in withDummyFetchers[Id] {
        implicit extraBlockFetcher => implicit quorumFetcher =>
          implicit proposalFetcher =>
            //given

            //test dataFetcher for blocks, returns a single response
            implicit val blockFetcher = testBlockFetcher[Id](
              (_, _) => TezosResponseBuilder.genesisBlockResponse
            )

            //when
            val block: TezosTypes.Block =
              sut.getBlock[Id](BlockHash("BLockGenesisGenesisGenesisGenesisGenesisb83baZgbyZe"))._1
  ...
  "getLatestBlocks" should "correctly fetch all the blocks if no depth is passed-in" in withDummyFetchers[IO] {
        implicit extraBlockFetcher => implicit quorumFetcher =>
          implicit proposalFetcher =>
            ...
            implicit val blockFetcher =
              testBlockFetcher[IO] {
                case (hash, offset) =>
                  //an array can be used as a function where the index is the input
                  TezosResponseBuilder.precannedGetBlockQueryResponse(offset)
              }
  ...
  "getLatestBlocks" should "correctly fetch blocks starting from a given head" in withDummyFetchers[IO] {
        implicit extraBlockFetcher => implicit quorumFetcher =>
          implicit proposalFetcher =>
            ...
            implicit val blockFetcher =
              testBlockFetcher[IO] {
                case (hash, offset) if hash == referenceHash =>
                  //an array can be used as a function where the index is the input
                  TezosResponseBuilder.precannedGetBlockQueryResponse(offset + 1)
                case (hash, _) =>
                  //the expected reference hash is wrong, return something bad
