...
class AccountsTaggingProcessTest extends WordSpec with Matchers with OptionValues {
  ...
  type SyncState[T] = StateT[SyncIO, Option[BlockLevel], T]

  val sut = AccountsTaggingProcess[SyncState]

  "the accounts tagging process" should {

      "return no flags if no operations exists" in {
        makeApi().map { implicit api =>
          val (reached, flagged) = sut.process.run(Option.empty).unsafeRunSync()

          reached.value shouldBe 0
      ...
      "return the count of flags and mark the correct account ids if operations are found" in {
        ...
        makeApi(exampleOperations).map { implicit api =>
          ...
          reached.value shouldBe exampleOperations.map(_.blockLevel).max
      ...
      "not flag accounts based on operations with lower block level than requested" in {

        val highestLevel = exampleOperations.map(_.blockLevel).max

        makeApi(exampleOperations).map { implicit api =>
          val (reached, flagged) = sut.process.run(Some(highestLevel)).unsafeRunSync()

          reached.value shouldBe highestLevel
          flagged.value shouldBe 0
  ...
  trait RecordingApiStub extends sut.ServiceDependencies {
    ...
    //Converts from one wrapper to another, for any content type
    val liftSyncState: SyncIO ~> SyncState = StateT.liftK[SyncIO, sut.ProcessorInput]

    val activated: Ref[SyncIO, List[PublicKeyHash]]
    val revealed: Ref[SyncIO, List[PublicKeyHash]]

    override def readHighWatermark: SyncState[BigDecimal] = liftSyncState(SyncIO(0))
    ...
    override def flagAsActive(accountId: PublicKeyHash): SyncState[Int] =
      liftSyncState(activated.modify(recordPkh(accountId)))

    override def flagAsRevealed(accountId: PublicKeyHash): SyncState[Int] =
      liftSyncState(revealed.modify(recordPkh(accountId)))

    override def readOperations(fromLevel: BigDecimal): fs2.Stream[SyncState, OperationsRow] =
      fs2.Stream.empty.covary[SyncState]
  }
