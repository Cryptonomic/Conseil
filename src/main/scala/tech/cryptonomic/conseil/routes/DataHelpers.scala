...
trait DataHelpers
    extends QueryStringListsServer
    with Validation
  ...
  def anyEncoder: Encoder[Any] =
    (a: Any) =>
      a match {
        ...
        case x: scala.collection.immutable.Vector[Tables.OperationGroupsRow] =>
          x.map(_.asJson(operationGroupsRowSchema.encoder)).asJson
  ...
  implicit override def queryResponseSchema: JsonSchema[List[QueryResponse]] =
    new JsonSchema[List[QueryResponse]] {
      override def encoder: Encoder[List[QueryResponse]] =
        (a: List[QueryResponse]) =>
          a.map { myMap =>
            Json.obj(
              myMap
                .map(
  ...
  implicit override def blocksByHashSchema: JsonSchema[AnyMap] = new JsonSchema[AnyMap] {
    override def encoder: Encoder[AnyMap] =
      (a: AnyMap) =>
        Json.obj(a.map {
  ...
  implicit override def timestampSchema: JsonSchema[Timestamp] = new JsonSchema[Timestamp] {
