...
/** How to use, based on tezos examples
  ...
  * - fetchJson function, that, given a collection of inputs, returns the effectful result of json data paired with its input value (correlation)
  * - decodeJson function that will decode every single json value to a full-blown scala data type
  *
  ...
  *
  * This enables us to easily compose the `fetchJson` function and `decodeJson` function to a general `fetch` function, that will use the former to retrieve a list of
  * json values, paired with input, and apply the decoding to each, all the while maintainig the correlation with the original input value.
  *
  * In addition to this, we might want to apply multiple decoding of json, and pair the decoded results with the same inputs
  ...
  * This is where the `addDecoding` and `typledResults` combinators comes useful.
  */
object MultiFetchDecoding {
  ...
  /** Adds an extra decoding operation for every intermediate encoded result, returning both the
    ...
    * We then addDecoding[Future, List, BlockHash, List[Operations], List[AccountId], String](operationsFetcher, accountDecoder)
    * The fetcher contains both the fetch operation and a single decding function, while the `additionalDecode` is the second decoding function.
    ...
    * - returns the decodings in a tuple, each one being also paired to the corresponding input hash
    */
  ...
  /** Combines two multifetch acting on the same inputs to get a 3-tuple of the outputs and merge them all
    * in a single output value, using a `merge` function of the three values
    *
    * Example:
  ...
  /** Combines three multifetch acting on the same inputs to get am arbitrary combination of the outputs,
    ...
    * of block hashes, we might want to directly take the three corresponding values and combine them in a resulting value
    * instead of simply creating a 3-tuple. In our case we might want to build a CurrentVotes object using the three params.
    * This is complicated because we receive three Lists, containing the "hash to value" pair, and we need to associate each
    * value in the list to the same input on the others.
    */
  ...
  /** Allows to add an additional decoder to a "multi-fetch" instance, by providing an extension method `decodeAlso(additionalDecode)`,
    ...
    * The result is the same as calling `MultiFetchDecoding.addDecoding` method
    */
  object Syntax {
