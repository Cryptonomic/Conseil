...
package tezos {
  case class CustomNodeInterface(override val streamingRequestsConnectionPooling: ConnectionPoolSettings)
    (implicit system: ActorSystem, tezosConf: TezosConfiguration, reqConf: NetworkCallsConfiguration, clientConf: HttpStreamingConfiguration)
    ...
    loadedConf.foreach {
      conf =>
        ...
        downloadRange foreach {
          ...
          node.runBatchedGetQuery(network, blocksUrls, identity[String], concurrency)
          .onComplete {
