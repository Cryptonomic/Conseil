...
object Lorre extends App with TezosErrors with LazyLogging with LorreAppConfig with LorreOutput {
  ...
  private[this] def processTezosBlocks(): Future[Done] = {
    ...
    val blockPagesToSynchronize = lorreConf.depth match {
      case Newest => getBlocksNotInDatabase()
      case Everything => getLatestBlocks()
      case Custom(n) => getLatestBlocks(Some(n), lorreConf.headHash)
    }

    /* will store a single page of block results */
    def processBlocksPage(results: Future[BlockFetchingResults]): Future[Int] =
      results.flatMap {
        blocksWithAccounts =>

          val (mainChainBlocksWithAction, forkedBlocksWithAction) = blocksWithAccounts.partition(b => b._1.action == WriteBlock)
          val (invalidatedBlocksWithAction, revalidatedBlocksWithAction) = forkedBlocksWithAction.partition(b => b._1.action == WriteAndInvalidateBlock)

          val mainChainBlocks = mainChainBlocksWithAction.map{
            case (BlockWithAction(block, _), accounts) => block -> accounts
          }
          val invalidatedBlocks = invalidatedBlocksWithAction.map{
            case (BlockWithAction(block, _), accounts) => block -> accounts
          }
          val revalidatedBlocks = revalidatedBlocksWithAction.map{
            case (BlockWithAction(block, _), accounts) => block -> accounts
          }
          ...
          //currently the invalidation db-operations are not implemented
          for {
            _ <- db.run(TezosDb.writeBlocksAndCheckpointAccounts(mainChainBlocks.toMap)) andThen logBlockOutcome
            _ <- processVotesForBlocks(mainChainBlocks.map{case (block, _) => block}) andThen logVotingOutcome
            // _ <- DBIOAction.sequence(
            //       Seq(db.run(TezosDb.writeAndInvalidateBlockIO(invalidatedBlocks), TezosDb.revalidateBlocksIO(revalidatedBlocks))
            //     )
