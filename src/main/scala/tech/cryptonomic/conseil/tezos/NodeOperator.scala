...
class NodeOperator(val network: String, batchConf: BatchFetchConfiguration) extends LazyLogging {
  ...
  def getAccountsForBlock[F[_]: MonadThrow](accountIds: List[AccountId], blockHash: BlockHash)(
    ...
    implicit val accountFetcher: DataFetcher.Aux[F, Throwable, AccountId, Option[Account], String] = fetchProvider(
      blockHash
    )
  ...
  private def getBlockRelatedEntities[F[_]: ApplicativeThrow: Concurrent, Key, Entity](
    ...
    keyStreamsByBlock.map {
      ...
      .groupAdjacentBy {
        case (key, entity) =>
          keyIndex(key) //create chunks having the same block reference, relying on how the stream is ordered
      }
      .map {
  ...
  def getAccountsForBlock[F[_]: ApplicativeThrow: Concurrent](
    ...
    def parseMichelsonScripts(id: AccountId): Account => Account =
      withLoggingContext[Account, String](makeContext = _ => s"account with key hash ${id.id}") { implicit ctx =>
        val scriptAlter = scriptLens.modify(toMichelsonScript[MichelsonSchema, String])
        val storageAlter = storageLens.modify(toMichelsonScript[MichelsonInstruction, String])
     ...
     */
    accountIds
      .parEvalMap(batchConf.accountFetchConcurrencyLevel)(
    ...
    getBlockRelatedEntities[F, AccountId, Account](
      ...
      loadEntitiesByBlock = fetchProvider.map {
        implicit fetcher => //pass this implicitly from the reader
          (ids: Stream[F, AccountId]) =>
            getAccountsForBlock(ids)
      },
  ...
  def getDelegatesForBlock[F[_]: ApplicativeThrow: Concurrent](
     ...
     */
    delegateKeys
      .parEvalMap(batchConf.delegateFetchConcurrencyLevel)(
        fetcher.tapWith((_, _)).run
      )
      .onError(logError)
      .evalTap {
  ...
  def getDelegates[F[_]: MonadThrow: Concurrent](
    ...
    getBlockRelatedEntities[F, PublicKeyHash, Delegate](
      ...
      loadEntitiesByBlock = fetchProvider.map {
        implicit fetcher => //pass this implicitly from the reader
          (keys: Stream[F, PublicKeyHash]) =>
            getDelegatesForBlock(keys)
      },
  ...
  def getVotingDetails[F[_]: MonadThrow](block: Block)(
    ...
    val fetchProposals =
      fetcher[F, Block, List[ProtocolId], Throwable].tapWith {
  ...
  private def getBlocks[F[_]: MonadThrow: Concurrent](
    ...
    logger.debug(
      s"Request to fetch blocks in levels $levelRange, with reference block at level $levelRef and hash: ${hashRef.value}"
    )
