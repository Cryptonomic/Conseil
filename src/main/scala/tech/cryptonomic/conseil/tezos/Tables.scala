...
trait Tables {
   ...
   *  @param bigMapId Database column big_map_id SqlType(numeric), Default(None)
  ...
  case class BigMapContentsRow(bigMapId: Option[scala.math.BigDecimal] = None, key: Option[String] = None, keyHash: Option[String] = None, value: Option[String] = None)
  /** GetResult implicit for fetching BigMapContentsRow objects using plain SQL queries */
  implicit def GetResultBigMapContentsRow(implicit e0: GR[Option[scala.math.BigDecimal]], e1: GR[Option[String]]): GR[BigMapContentsRow] = GR{
    prs => import prs._
  ...
  class BigMapContents(_tableTag: Tag) extends profile.api.Table[BigMapContentsRow](_tableTag, Some("tezos"), "big_map_contents") {
    ...
    /** Database column big_map_id SqlType(numeric), Default(None) */
    val bigMapId: Rep[Option[scala.math.BigDecimal]] = column[Option[scala.math.BigDecimal]]("big_map_id", O.Default(None))
    ...
    lazy val bigMapsFk = foreignKey("big_map_contents_id_fkey", bigMapId, BigMaps)(r => Rep.Some(r.bigMapId), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
