...
trait Tables {
  ...
  /** Entity class storing rows of table EndorsingRights
    ...
    *  @param estimatedTime Database column estimated_time SqlType(timestamp) */
  case class EndorsingRightsRow(blockHash: String, level: Int, delegate: String, slots: String, estimatedTime: java.sql.Timestamp)
  ...
  implicit def GetResultEndorsingRightsRow(implicit e0: GR[String], e1: GR[Int], e2: GR[java.sql.Timestamp]): GR[EndorsingRightsRow] = GR{
    prs => import prs._
      EndorsingRightsRow.tupled((<<[String], <<[Int], <<[String], <<[String], <<[java.sql.Timestamp]))
  }
  ...
  class EndorsingRights(_tableTag: Tag) extends profile.api.Table[EndorsingRightsRow](_tableTag, "endorsing_rights") {
    def * = (blockHash, level, delegate, slots, estimatedTime) <> (EndorsingRightsRow.tupled, EndorsingRightsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(blockHash), Rep.Some(level), Rep.Some(delegate), Rep.Some(slots), Rep.Some(estimatedTime))).shaped.<>({r=>import r._; _1.map(_=> EndorsingRightsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val pk = primaryKey("endorsing_rights_pkey", (level, delegate, slots))
