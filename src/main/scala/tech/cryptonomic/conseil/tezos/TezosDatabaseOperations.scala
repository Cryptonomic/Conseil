...
object TezosDatabaseOperations extends LazyLogging {
  ...
  /**
    ...
    */
  def writeBakingRights(bakingRightsMap: Map[(Option[Int], Option[Int], BlockHash), List[BakingRights]]): DBIO[Option[Int]] = {
  ...
  /**
    ...
    */
  def writeEndorsingRights(endorsingRightsMap: Map[(Option[Int], Option[Int], BlockHash), List[EndorsingRights]]): DBIO[Option[Int]] = {
  ...
  def selectWithPredicates(
    ...
    val q = (temporalPartition, snapshot) match {
      case (Some(tempPartition), Some(snap)) =>
        makeTemporalQuery(table, columns, predicates, aggregation, ordering, tempPartition, snap, limit)
      case _ =>
