...
/** Adds more specific API functionalities to perform on a tezos node,
  ...
  */
class NodeSenderOperations(network: String, batchConf: BatchFetchConfiguration, sodiumConf: SodiumConfiguration)
    extends NodeOperations(network, batchConf) {
  import com.muquit.libsodiumjna.{SodiumKeyPair, SodiumLibrary, SodiumUtils}
  import NodeSenderOperations._
