...
import tech.cryptonomic.conseil.util.JsonUtil.{JsonString, fromJson, toJson}
...
class NodeSenderOperator(network: String, batchConf: BatchFetchConfiguration, sodiumConf: SodiumConfiguration)
  ...
  def forgeOperations[Eff[_] : PostHandler : Functor](
    ...
    RpcHandler.runPost("/blocks/head/proto/helpers/forge/operations", Some(toJson(payload)))
  ...
  def applyOperation[Eff[_] : PostHandler : Functor](
    ...
    signedOpGroup: SignedOperationGroup): Eff[AppliedOperation] = {
    ...
    RpcHandler.runPost("/blocks/head/proto/helpers/apply_operation", Some(toJson(payload)))
      .map { result =>
  ...
  def injectOperation[Eff[_] : PostHandler : Functor](signedOpGroup: SignedOperationGroup): Eff[String] = {
    ...
    RpcHandler.runPost("/inject_operation", Some(toJson(payload)))
