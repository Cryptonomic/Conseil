...
CREATE OR REPLACE FUNCTION public.truncate_tables(username character varying) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE TABLE IF NOT EXISTS public.operation_groups (
    protocol character varying NOT NULL,
    chain_id character varying,
    hash character varying NOT NULL,
...
CREATE SEQUENCE IF NOT EXISTS public.operations_operation_id_seq
    START WITH 177489
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE SEQUENCE IF NOT EXISTS public.operations_operation_id_seq1
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE INDEX IF NOT EXISTS fki_block ON public.operation_groups USING btree (block_id);
...
CREATE INDEX IF NOT EXISTS fki_fk_blockhashes ON public.operations USING btree (block_hash);
