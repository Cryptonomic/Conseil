package tech.cryptonomic.conseil.common.tezos.michelson.parser

import org.scalatest._
import tech.cryptonomic.conseil.common.tezos.michelson.dto._
import tech.cryptonomic.conseil.common.tezos.michelson.parser.JsonParser.{parse, ParserError}
...
class JsonParserSpec extends FlatSpec with Matchers {
  ...
  it should "parse two-argument MichelsonType" in {
      ...
      parse[MichelsonExpression](json) should equal(
        Right(MichelsonType("pair", List(MichelsonType("int"), MichelsonType("address"))))
      )
    }
      ...
      parse[MichelsonExpression](json) should equal(
        Right(
          MichelsonType(
            "contract",
            List(
              MichelsonType("or", List(MichelsonType("option", List(MichelsonType("address"))), MichelsonType("int")))
            )
          )
        )
      )
  ...
  it should "parse MichelsonType with annotation" in {
      ...
      parse[MichelsonExpression](json) should equal(Right(MichelsonType(prim = "int", annotations = List(":p"))))
    }

  it should "parse MichelsonInstruction with only one simple instruction" in {
      val json = """{"prim": "DUP"}"""

      parse[MichelsonInstruction](json) should equal(Right(MichelsonSingleInstruction("DUP")))
    }
  ...
  it should "parse MichelsonInstructionSequence" in {
      ...
      parse[MichelsonInstruction](json) should equal(
        Right(MichelsonInstructionSequence(List(MichelsonSingleInstruction("CDR"), MichelsonSingleInstruction("DUP"))))
      )
    }
  ...
  it should "parse typed MichelsonInstruction" in {
      ...
      parse[MichelsonInstruction](json) should equal(
        Right(MichelsonSingleInstruction("NIL", List(MichelsonType("operation"))))
      )
    }
  ...
  it should "parse MichelsonInstruction with annotation" in {
      ...
      parse[MichelsonInstruction](json) should equal(
        Right(MichelsonSingleInstruction("CAR", annotations = List("@pointcolor")))
      )
    }
  ...
  it should "parse MichelsonInstruction typed with string data" in {
      ...
      parse[MichelsonInstruction](json) should equal(
        Right(
          MichelsonInstructionSequence(
            List(MichelsonSingleInstruction("PUSH", List(MichelsonType("mutez"), MichelsonStringConstant("0"))))
          )
        )
      )
    }
  ...
  it should "parse MichelsonInstruction typed with bytes data" in {
      ...
      parse[MichelsonInstruction](json) should equal(
        Right(MichelsonSingleInstruction("PUSH", List(MichelsonType("bytes"), MichelsonBytesConstant("0500"))))
      )
    }
  ...
  it should "parse empty MichelsonInstruction when it appears alone" in {
      ...
      parse[MichelsonInstruction](json) should equal(
        Right(
          MichelsonInstructionSequence(List(MichelsonSingleInstruction("IF_NONE", List(MichelsonEmptyInstruction))))
        )
      )
    }
  ...
  it should "parse MichelsonCode" in {
      ...
      parse[MichelsonCode](json) should equal(Right(MichelsonCode(List(MichelsonSingleInstruction("DUP")))))
    }
            ...
            MichelsonType(
              ...
              List(
                MichelsonType(
                  "or",
                  List(
                    MichelsonType("option", List(MichelsonType("address"))),
                    MichelsonType(
                      "or",
                      List(
                        MichelsonType(
                          ...
                          List(
                            MichelsonType("option", List(MichelsonType("address"))),
                            MichelsonType("option", List(MichelsonType("mutez")))
                          )
                        ),
                        MichelsonType(
                          "or",
