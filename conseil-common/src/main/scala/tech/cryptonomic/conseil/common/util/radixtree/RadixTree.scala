...
import tech.cryptonomic.conseil.common.util.radixtree.RadixTree.Key
...
final class RadixTree[K, V](val prefix: K, private[radixtree] val children: Array[RadixTree[K, V]], private[radixtree] val valueOpt: Option[V]) extends NoEquals {
  ...
  def keys(implicit K: Key[K]): Iterable[K] = new AbstractIterable[K] {
    override def foreach[U](f: K => U) = foreachKey(K.empty, f)
  ...
  private def modifyOrRemove0(f: (K, V, Int) => Option[V], prefix: K)(implicit K: Key[K]): RadixTree[K, V] = {
    ...
    val valueOpt1 = f(newPrefix, valueOpt.get, children1.length)
  ...
  private def copy(
      ...
      valueOpt: Option[V] = this.valueOpt,
  ...
  def get(key: K)(implicit K: Key[K]): Option[V] = get0(key, 0)

  def getOrNull(key: K)(implicit K: Key[K]): V = get0(key, 0).get
  ...
  @tailrec
  private def get0(key: K, offset: Int)(implicit K: Key[K]): Option[V] =
    ...
    if (fd == maxFd) {
      ...
      val children1 =
        if (K.compareAt(childA.prefix, 0, childB.prefix, 0) < 0)
          ...
          Array(childB, childA)
      new RadixTree[K, V](commonPrefix, children1, Option.empty)
    }
  }
  ...
  def filterKeysContaining(fragment: K)(implicit K: Key[K], v: Hash[V]) = {
    ...
    def filter0(prefix: K, tree: RadixTree[K, V]): RadixTree[K, V] = {
      ...
      if (K.indexOf(prefix1, fragment) >= 0) tree
        ...
        tree.copy(valueOpt = Option.empty[V], children = children2)
      }
    }
...
object RadixTree {
  ...
  def empty[K: Key, V](implicit K: Key[K]): RadixTree[K, V] =
    new RadixTree[K, V](K.empty, emptyChildren[K, V], Option.empty)

  def singleton[K: Key, V](key: K, value: V): RadixTree[K, V] =
    new RadixTree[K, V](key, RadixTree.emptyChildren[K, V], Option(value))
