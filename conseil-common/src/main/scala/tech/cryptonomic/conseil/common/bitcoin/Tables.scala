...
trait Tables {
  ...
  case class BlocksRow(hash: String, confirmations: Int, size: Int, weight: Int, height: Int, version: Int, merkleRoot: String, time: java.sql.Timestamp, nonce: Long, bits: String, difficulty: Long, nTx: Int, previousBlockHash: Option[String] = None, nextBlockHash: Option[String] = None)
  /** GetResult implicit for fetching BlocksRow objects using plain SQL queries */
  implicit def GetResultBlocksRow(implicit e0: GR[String], e1: GR[Int], e2: GR[java.sql.Timestamp], e3: GR[Long], e4: GR[Option[String]]): GR[BlocksRow] = GR{
    prs => import prs._
    BlocksRow.tupled((<<[String], <<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[String], <<[java.sql.Timestamp], <<[Long], <<[String], <<[Long], <<[Int], <<?[String], <<?[String]))
  }
  ...
  class Blocks(_tableTag: Tag) extends profile.api.Table[BlocksRow](_tableTag, Some("bitcoin"), "blocks") {
    def * = (hash, confirmations, size, weight, height, version, merkleRoot, time, nonce, bits, difficulty, nTx, previousBlockHash, nextBlockHash) <> (BlocksRow.tupled, BlocksRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(hash), Rep.Some(confirmations), Rep.Some(size), Rep.Some(weight), Rep.Some(height), Rep.Some(version), Rep.Some(merkleRoot), Rep.Some(time), Rep.Some(nonce), Rep.Some(bits), Rep.Some(difficulty), Rep.Some(nTx), previousBlockHash, nextBlockHash)).shaped.<>({r=>import r._; _1.map(_=> BlocksRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get, _10.get, _11.get, _12.get, _13, _14)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  case class InputsRow(txid: String, vOut: Option[Int] = None, scriptSigAsm: Option[String] = None, scriptSigHex: Option[String] = None, sequence: Long, coinbase: Option[String] = None, txInWitness: Option[String] = None)
  /** GetResult implicit for fetching InputsRow objects using plain SQL queries */
  implicit def GetResultInputsRow(implicit e0: GR[String], e1: GR[Option[Int]], e2: GR[Option[String]], e3: GR[Long]): GR[InputsRow] = GR{
    prs => import prs._
  ...
  class Inputs(_tableTag: Tag) extends profile.api.Table[InputsRow](_tableTag, Some("bitcoin"), "inputs") {
    def * = (txid, vOut, scriptSigAsm, scriptSigHex, sequence, coinbase, txInWitness) <> (InputsRow.tupled, InputsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(txid), vOut, scriptSigAsm, scriptSigHex, Rep.Some(sequence), coinbase, txInWitness)).shaped.<>({r=>import r._; _1.map(_=> InputsRow.tupled((_1.get, _2, _3, _4, _5.get, _6, _7)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val txInWitness: Rep[Option[String]] = column[Option[String]]("tx_in_witness", O.Length(2147483647,varying=false), O.Default(None))

    /** Foreign key referencing Transactions (database name bitcoin_inputs_txid_fkey) */
    lazy val transactionsFk = foreignKey("bitcoin_inputs_txid_fkey", txid, Transactions)(r => r.txid, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
  ...
  class Outputs(_tableTag: Tag) extends profile.api.Table[OutputsRow](_tableTag, Some("bitcoin"), "outputs") {
    ...
    val scriptPubKeyAddresses: Rep[Option[String]] = column[Option[String]]("script_pub_key_addresses", O.Length(2147483647,varying=false), O.Default(None))

    /** Foreign key referencing Transactions (database name bitcoin_outputs_txid_fkey) */
    lazy val transactionsFk = foreignKey("bitcoin_outputs_txid_fkey", txid, Transactions)(r => r.txid, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
   ...
   *  @param confirmations Database column confirmations SqlType(int4)
   *  @param time Database column time SqlType(timestamp) */
  case class TransactionsRow(txid: String, blockhash: String, hash: String, size: Int, weight: Int, version: Int, confirmations: Int, time: java.sql.Timestamp)
  /** GetResult implicit for fetching TransactionsRow objects using plain SQL queries */
  implicit def GetResultTransactionsRow(implicit e0: GR[String], e1: GR[Int], e2: GR[java.sql.Timestamp]): GR[TransactionsRow] = GR{
    prs => import prs._
    TransactionsRow.tupled((<<[String], <<[String], <<[String], <<[Int], <<[Int], <<[Int], <<[Int], <<[java.sql.Timestamp]))
  }
  /** Table description of table transactions. Objects of this class serve as prototypes for rows in queries. */
  class Transactions(_tableTag: Tag) extends profile.api.Table[TransactionsRow](_tableTag, Some("bitcoin"), "transactions") {
    def * = (txid, blockhash, hash, size, weight, version, confirmations, time) <> (TransactionsRow.tupled, TransactionsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(txid), Rep.Some(blockhash), Rep.Some(hash), Rep.Some(size), Rep.Some(weight), Rep.Some(version), Rep.Some(confirmations), Rep.Some(time))).shaped.<>({r=>import r._; _1.map(_=> TransactionsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val blocksFk = foreignKey("bitcoin_transactions_blockhash_fkey", blockhash, Blocks)(r => r.hash, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
