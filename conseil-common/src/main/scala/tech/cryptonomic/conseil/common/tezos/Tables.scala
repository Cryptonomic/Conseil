...
trait Tables {
  ...
  /** Entity class storing rows of table Nfts
    ...
    *  @param nftAddress Database column nft_address SqlType(text)
  ...
  implicit def GetResultNftsRow(implicit e0: GR[String], e1: GR[Int], e2: GR[Option[java.sql.Timestamp]]): GR[NftsRow] =
    GR { prs =>
      import prs._
      NftsRow.tupled(
        (
  ...
  class Nfts(_tableTag: Tag) extends profile.api.Table[NftsRow](_tableTag, Some("tezos"), "nfts") {
    ...
    def ? =
      (
        ...
        { r =>
          import r._; _1.map(_ => NftsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9)))
        },
        (_: Any) => throw new Exception("Inserting into ? projection not supported.")
      )
    ...
    /** Database column nft_address SqlType(text) */
    val nftAddress: Rep[String] = column[String]("nft_address")
