...
/** Here we verify the properties of the [[ForkDetector]] implementation.
  ...
  * generated by the test-harness to prove that a given set of assumptions are
...
class ForkDetectorSpec extends AnyPropSpec with ScalaCheckDrivenPropertyChecks with Matchers {
  ...
  private object Fixtures {
     ...
     * We chose to encode two alternate values as an `Either` type since we don't care about
     * the actual content, only to differentiate whether it's one or the other.
     * Thus we can put a Unit value in the Either, which doesn't force any
     * choice of value as content, being as arbitrary as any other.
     */
