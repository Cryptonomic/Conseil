...
class TezosDatabaseOperationsTest
  ...
  "The database api" should {
    ...
    "write blocks" in {
      ...
      val generatedBlocks = basicBlocks.zipWithIndex map { case (block, idx) =>
      ...
      whenReady(dbHandler.run(sut.writeBlocks(generatedBlocks, noTokenContracts))) { _ =>
        ...
        forAll(dbBlocks zip generatedBlocks) { case (row, block) =>
        ...
        forAll(dbBlocksAndGroups) { case (blockRow, groupRow) =>
        ...
        forAll(dbOperations) { case (groupRow, opRow) =>
    ...
    "write accounts for a single block" in {
      ...
      forAll(dbAccounts zip accountsInfo.content) { case (row, (id, account)) =>
    ...
    "store checkpoint account ids with block reference" in {
      ...
      val flattenedIdsData = ids.flatMap { case (hash, level, time, cycle, period, accounts) =>
        accounts.map((hash, level, _))
      }

      forAll(checkpointRows.zip(flattenedIdsData)) { case (row, (hash, level, accountId)) =>
    ...
    "write bakers for a single block" in {
      ...
      forAll(dbDelegates zip delegatesInfo.content) { case (row, (pkh, delegate)) =>
    ...
    "update bakers if they exists already" in {
      ...
      //both rows on db should refer to updated data
      forAll(dbDelegates zip updatedDelegates.content) { case (row, (pkh, delegate)) =>
        row.pkh shouldEqual pkh.value
        row.balance shouldEqual (delegate.balance match {
          case PositiveDecimal(value) => Some(value)
          case _ => None
    ...
    "store checkpoint delegate key hashes with block reference" in {
      ...
      val flattenedKeysData = keys.flatMap { case (hash, level, time, cycle, period, keys) =>
        keys.map((hash, level, _))
      }

      forAll(checkpointRows.zip(flattenedKeysData)) { case (row, (hash, level, keyHash)) =>
        row.blockId shouldEqual hash.value
        row.blockLevel shouldBe level
    ...
    "update the accounts listed as bakers" in {
      ...
      val historyAccounts = (delegate :: delegatedAccounts).map(row =>
    ...
    "read all distinct account ids and add entries for each in the checkpoint" in {
      ...
      import org.scalatest.Inspectors._
      forAll(checkpoint.values) { case BlockReference(hash, level, instantOpt, cycleOpt, periodOpt) =>
