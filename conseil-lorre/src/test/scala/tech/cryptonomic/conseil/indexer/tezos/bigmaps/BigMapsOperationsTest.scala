...
class BigMapsOperationsTest
  ...
  "The big-maps operations" should {
    ...
    "save big map diffs updates contained in a list of blocks" in {
      ...
      //use combinators defined in the fixtures to update big_map_diff values within lists of operations
      val updateMap: ListTransf[Operation] = Operations.updateOperationsWithBigMapUpdate { case (_: Transaction) =>
    ...
    "save big map diffs references to originated accounts in a list of blocks" in {
      ...
      accounts should contain theSameElementsAs sampleAccountIds.map(id =>
        OriginatedAccountMapsRow(
          bigMapId = BigDecimal(1),
          accountId = id,
          blockLevel = Some(1L),
    ...
    "save token balance updates for corresponding big map updates on transactions to a token contract" in {
      ...
      val updateTransactionDestination: ListTransf[Operation] = Operations.modifyTransactions { case transaction =>
        transaction.copy(
    ...
    "update with only the latest big map content for diffs having the same target map and key: Issue#807" in {
      ...
      val (operationsWithDiffs: List[List[OperationsGroup]], opGroupHashes: List[OperationHash]) = blocks.map { block =>
        val sampleOperations = generateOperationGroup(block, generateOperations = true)
        val opGroupHash = sampleOperations.hash
      ...
      val blocksToSave =
        blocks.zip(operationsWithDiffs).map { case (block, ops) =>
          block.copy(operationGroups = ops)
        }
    ...
    "copy big map contents for diffs contained in a list of blocks" in {
      ...
      val copyMap: ListTransf[Operation] = Operations.updateOperationsWithBigMapCopy { case (_: Transaction) =>
        Contract.BigMapCopy(
          action = "copy",
    ...
    "delete all data in selected big maps for diffs contained in a list of blocks" in {
      ...
      val initialOriginatedReferences = sampleAccountIds.map(id =>
        OriginatedAccountMapsRow(
          bigMapId = BigDecimal(1),
          accountId = id,
          forkId = Fork.mainForkId
      ...
      val removeMap: ListTransf[Operation] = Operations.updateOperationsWithBigMapRemove { case (_: Transaction) =>
