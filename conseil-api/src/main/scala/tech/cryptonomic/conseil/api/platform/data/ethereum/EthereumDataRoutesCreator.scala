...
trait EthereumDataRoutesCreator extends EthereumDataEndpoints with ApiDataRoutes {
  ...
  def delegateCall0[B](
    ...
    endpoint.serverLogic[IO] { case (network, _) =>
      platformNetworkValidation(network)(f).map(Right.apply)
    }
  ...
  def delegateCall[A, B](
    ...
    endpoint.serverLogic[IO] { case ((network, element), _) =>
      platformNetworkValidation(network)(f(element)).map(Right.apply)
    }
