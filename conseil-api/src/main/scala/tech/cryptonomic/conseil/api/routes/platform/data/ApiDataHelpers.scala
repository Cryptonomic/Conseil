...
trait ApiDataHelpers extends server.Endpoints with server.JsonSchemaEntities {
  ...
  protected def fieldJsonSchema(formattedFieldSchema: JsonSchema[FormattedField]): JsonSchema[Field] =
