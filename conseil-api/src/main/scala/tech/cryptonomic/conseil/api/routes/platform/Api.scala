...
import tech.cryptonomic.conseil.api.routes.platform.data.tezos.{TezosDataEndpoint, TezosDataOperations}
...
trait Api {
  def dataEndpoint(metadataService: MetadataService): ApiDataEndpoints
...
class TezosApi(metadataOverrides: MetadataConfiguration, server: ConseilConfiguration)(
  ...
  override def dataEndpoint(metadataService: MetadataService): ApiDataEndpoints =
    TezosDataEndpoint(metadataService, metadataOverrides, dataOperations, server.maxQueryResultSize)(apiDispatcher)
}
